...
#include <render_base/PredefinedVertexAttributes.h>

NS_BEGIN

template <typename T>
int buildBox(std::vector<T>& verts,
...
BoxGeometry::BoxGeometry(float width,
                         float height,
                         float depth,
                         int widthSegments,
                         int heightSegments,
                         int depthSegments,
                         BufferAttributes::StockAttributes vertexAttribute)
{
    m_width = width;
    m_height = height;
    m_depth = depth;
