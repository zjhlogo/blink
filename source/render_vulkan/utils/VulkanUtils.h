...
namespace blink
{
    ...
    {
    public:
        static void enumerateInstanceExtensionProperties(std::vector<VkExtensionProperties>& propertiesOut,
                                                         const char* layerName = nullptr);
        static bool checkExtensionsSupported(const std::vector<VkExtensionProperties>& properties,
                                             const std::vector<const char*>& requiredExtensions);

        static void enumerateInstanceLayerProperties(std::vector<VkLayerProperties>& propertiesOut);
        static bool checkValidationLayerSupported(const std::vector<VkLayerProperties>& properties,
                                                  const std::vector<const char*>& requiredLayers);

        static void enumeratePhysicalDevices(std::vector<VkPhysicalDevice>& devicesOut, VkInstance instance);

        static void getPhysicalDeviceQueueFamilyProperties(std::vector<VkQueueFamilyProperties>& propertiesOut,
                                                           VkPhysicalDevice device);

        static void getSurfaceFormats(std::vector<VkSurfaceFormatKHR>& formatsOut,
                                      VkPhysicalDevice device,
                                      VkSurfaceKHR surface);
        static void getSurfacePresentModes(std::vector<VkPresentModeKHR>& presentModesOut,
                                           VkPhysicalDevice device,
                                           VkSurfaceKHR surface);
        ...
        static uint32_t findMemoryType(const VkPhysicalDeviceMemoryProperties& memProperties,
                                       uint32_t typeFilter,
