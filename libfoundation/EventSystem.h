...
namespace blink
{
    ...
    {
    public:
        EventCallback(const std::function<void(T& evt)>& callback) :m_callback(callback) {};
        virtual ~EventCallback() {};

        virtual void invoke(Event& evt) override
        {
            m_callback(static_cast<T&>(evt));
        }
    ...
    public:
        ...
        public:
            ...
            void removeSender(EventHandler* sender)
            {
                m_senderSet.erase(sender);
            }
    ...
    public:
        ...
        template<typename T>
        bool registerEvent(EventTargetHost* host, void* target, const std::function<void(T& evt)>& callback)
        {
            EventInfo eventInfo;
...
#define DECLARE_EVENT_HOST() private: blink::EventHandler::EventTargetHost __event_host__
